...
- **A Ana Júlia NUNCA fala valor (decisão do Dr. Lucas, 22/07/2026).** `/api/agente/consultar-procedimentos` não seleciona nenhum campo de preço — a faixa não chega ao modelo, então nenhuma mudança de prompt pode fazê-la citar número. O único valor que aparece numa conversa é o que o Dr. Lucas respondeu para aquele paciente, via webhook. Motivo: a estimativa automática mandava a faixa genérica da mini lipo (R$ 8k–11k) para qualquer região, porque `extrairEstimativaDaConsulta` pegava o primeiro item da lista ordenada por nome. Junto disso saíram: a estimativa como caminho do funil, o agendamento a partir de estimativa aprovada e o estado `origem: "estimativa"`. **Não reintroduzir preço no retorno dessa rota.**
- **Preço por região:** tabela `procedimento_regioes` (migration `20260722180000`), cadastrada em Procedimentos → "Valores por região". É referência interna: entra na mensagem que o Dr. Lucas recebe no WhatsApp ao gerar orçamento (`src/lib/procedimentos/faixa-regiao.ts` → `notificar-handoff.ts`), nunca na conversa com o paciente. Se a região não tem faixa cadastrada, a seção não aparece — melhor sem número do que com número errado.
- **`src/lib/procedimentos/regioes.ts` é a FONTE ÚNICA das regiões anatômicas** (abdome, flancos, glúteo, braços, costas, coxas, culote, papada, mamas, pernas, axilas). Consumida pelo detector de handoff, pelo resolvedor de procedimento, pelo cadastro de preços e pela referência enviada ao Dr. Lucas. Para adicionar região, edite só `REGIOES_CORPO`.
- **Combo Paciente Modelo é resolvido por CONJUNTO de regiões, não por string.** `resolver-procedimento.ts` extrai as regiões do texto e decide: {abdome} → Só Abdome, {abdome, flancos} → sem enxerto, {abdome, flancos, glúteo} ou "enxerto" → completa. A lista antiga de strings casava `"abdome flancos"` mas não `"abdome e flancos"` — um "e" mandava o lead para o procedimento genérico.

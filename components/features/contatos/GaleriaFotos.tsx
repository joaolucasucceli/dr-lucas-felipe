...
interface GaleriaFotosProps {
  contatoId: string
  fotosIniciais: FotoContato[]
  isGestor: boolean
}

const TIPOS = [
  { valor: "antes", label: "Antes" },
  { valor: "depois", label: "Depois" },
...
export function GaleriaFotos({ contatoId, fotosIniciais, isGestor }: GaleriaFotosProps) {
  const [fotos, setFotos] = useState<FotoContato[]>(fotosIniciais)
  ...
  const [fotoAmpliada, setFotoAmpliada] = useState<{ foto: FotoContato; secao: FotoContato[] } | null>(null)
  ...
  async function handleUpload() {
    ...
    try {
      const res = await fetch(`/api/contatos/${contatoId}/fotos`, {
        method: "POST",
        body: formData,
      })
      if (!res.ok) {
      ...
      const novaFoto: FotoContato = await res.json()
  ...
  async function handleDeletar(fotoId: string) {
    setDeletando(true)
    try {
      const res = await fetch(`/api/contatos/${contatoId}/fotos/${fotoId}`, { method: "DELETE" })
  ...
  function CardFoto({ foto, secao }: { foto: FotoContato; secao: FotoContato[] }) {
    return (
      <div
  ...
  function Secao({ titulo, lista, cor }: { titulo: string; lista: FotoContato[]; cor?: string }) {
    ...
    <div className="space-y-6">
      ...
      {!temFotos ? (
        <EmptyState
          ...
          titulo="Nenhuma foto"
          descricao="Envie fotos do contato para acompanhamento antes/depois."
        />
      ) : (
